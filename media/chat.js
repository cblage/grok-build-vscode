(function () {
  ...
  function updateSandboxBtn() {
    ...
    if (!supported) {
      ...
      sandboxBtn.removeAttribute("aria-disabled");
    ...
    // Native `disabled` suppresses hover and keyboard focus. Keep the control
    // semantically locked with aria-disabled while allowing the rules inspector
    // to open in either state. The click handler remains an inert hard gate while
    // busy and opens the profile picker when selection is available.
    sandboxBtn.disabled = false;
    sandboxBtn.setAttribute("aria-disabled", disabled ? "true" : "false");
    sandboxBtn.classList.toggle("disabled", disabled);
    sandboxBtn.title = disabled
      ? `Sandbox: ${off ? "off (unsandboxed)" : cur} — selection locked; hover to inspect rules`
      : `Sandbox: ${off ? "off (unsandboxed)" : cur} — hover to inspect rules; click to ${off ? "pick" : "change"} profile`;
    if (sandboxPopover && !sandboxPopover.hidden && sandboxPopover.dataset.view === "rules") {
      renderSandboxRulesPopover();
    }
  }
  ...
  let sandboxRulesButtonHovered = false;
  let sandboxRulesPopoverHovered = false;
  let sandboxRulesHideTimer = null;

  function clearSandboxRulesHideTimer() {
    if (sandboxRulesHideTimer !== null) {
      clearTimeout(sandboxRulesHideTimer);
      sandboxRulesHideTimer = null;
    }
  }

  function closePopovers() {
    clearSandboxRulesHideTimer();
  ...
  function sandboxScopeMeta(scope) {
    return {
      off: { icon: ICON.unlock, description: "No OS sandbox" },
    ...
    }[scope] || { icon: ICON.account, description: "User-defined sandbox profile" };
  }

  function sandboxScopeFor(name, requestedScope) {
    if (name === "off") return "off";
    if (["workspace", "user", "builtin"].includes(requestedScope)) return requestedScope;
    return ["workspace", "devbox", "read-only", "strict"].includes(name) ? "builtin" : "user";
  }

  function sandboxRulesForCurrent() {
    const cur = state.sandboxCurrent || "off";
    if (state.sandboxRules && state.sandboxRules.name === cur) return state.sandboxRules;
    const option = (state.sandboxProfiles || []).find((candidate) => candidate.name === cur);
    const scope = sandboxScopeFor(cur, option?.scope);
    const base = ["off", "workspace", "devbox", "read-only", "strict"].includes(cur)
      ? cur
      : undefined;
    return {
      name: cur,
      scope,
      base,
      restrictNetwork: base === "read-only" || base === "strict",
      readOnly: [],
      readWrite: [],
      deny: [],
    };
  ...
  function sandboxBaseSummary(base) {
    return {
      off: {
        read: "Entire host filesystem",
        write: "Entire host filesystem",
      },
      workspace: {
        read: "Entire host filesystem",
        write: "Workspace, $GROK_HOME, and temporary storage",
      },
      devbox: {
        read: "Entire host filesystem",
        write: "Existing top-level directories except /data and virtual filesystems",
      },
      "read-only": {
        read: "Entire host filesystem",
        write: "$GROK_HOME and temporary storage",
      },
      strict: {
        read: "Workspace, $GROK_HOME, temporary storage, and essential runtime paths",
        write: "Workspace, $GROK_HOME, and temporary storage",
      },
    }[base] || {
      read: "Defined by the selected profile",
      write: "Defined by the selected profile",
    };
  }

  function sandboxRuleValue(value) {
    if (!Array.isArray(value)) return escapeHtml(value);
    return `<span class="sandbox-rule-paths">${value.map((path) => `<code>${escapeHtml(path)}</code>`).join("")}</span>`;
  }

  function sandboxRuleRow(kind, icon, label, value) {
    return (
      `<div class="sandbox-rule-row" data-sandbox-rule="${kind}">` +
        `<span class="sandbox-rule-icon">${icon}</span>` +
        `<span class="sandbox-rule-label">${escapeHtml(label)}</span>` +
        `<span class="sandbox-rule-value">${sandboxRuleValue(value)}</span>` +
      `</div>`
    );
  }

  function renderSandboxRulesPopover() {
    if (!sandboxPopover || !sandboxBtn) return;
    const rules = sandboxRulesForCurrent();
    const off = rules.name === "off";
    const scopeMeta = sandboxScopeMeta(rules.scope);
    const base = rules.base || (off ? "off" : undefined);
    const baseSummary = sandboxBaseSummary(base);
    const sourceIcons = off ? ICON.unlock : ICON.lock + scopeMeta.icon;
    let rows = "";
    if (rules.error) {
      rows += sandboxRuleRow("error", ICON.info, "Rules unavailable", rules.error);
    } else {
      rows += sandboxRuleRow(
        "base",
        ICON.terminalSecure,
        off ? "Boundary" : "Base profile",
        off ? "Disabled" : (rules.name === base ? `Built-in ${base}` : `Extends ${base}`),
      );
      rows += sandboxRuleRow("read", ICON.eye, "Read access", baseSummary.read);
      rows += sandboxRuleRow("write", ICON.pencil, "Write access", baseSummary.write);
      rows += sandboxRuleRow(
        "network",
        ICON.globe,
        "Network",
        rules.restrictNetwork
          ? "Restriction requested; not enforced by macOS Seatbelt"
          : "Unrestricted",
      );
      if (Array.isArray(rules.readOnly) && rules.readOnly.length) {
        rows += sandboxRuleRow("read-only", ICON.eye, "Read-only additions", rules.readOnly);
      }
      if (Array.isArray(rules.readWrite) && rules.readWrite.length) {
        rows += sandboxRuleRow("read-write", ICON.pencil, "Writable additions", rules.readWrite);
      }
      if (Array.isArray(rules.deny) && rules.deny.length) {
        rows += sandboxRuleRow("deny", ICON.eyeOff, "Denied paths", rules.deny);
      }
    }
    sandboxPopover.classList.add("sandbox-rules-popover");
    sandboxPopover.dataset.view = "rules";
    sandboxPopover.innerHTML =
      `<div class="sandbox-rules-header">` +
        `<span class="sandbox-rules-profile-icons">${sourceIcons}</span>` +
        `<span class="sandbox-rules-heading">` +
          `<strong>${escapeHtml(off ? "Sandbox off" : rules.name)}</strong>` +
          `<small>${escapeHtml(scopeMeta.description)} · fixed for this session</small>` +
        `</span>` +
      `</div>` +
      `<div class="sandbox-rules-list">${rows}</div>`;
  }
  ...
  function scheduleSandboxRulesPopoverClose() {
    clearSandboxRulesHideTimer();
    sandboxRulesHideTimer = setTimeout(() => {
      sandboxRulesHideTimer = null;
      if (
        sandboxPopover?.dataset.view === "rules" &&
        !sandboxRulesButtonHovered &&
        !sandboxRulesPopoverHovered &&
        document.activeElement !== sandboxBtn
      ) {
        sandboxPopover.hidden = true;
        delete sandboxPopover.dataset.view;
      }
    }, 100);
  }
  ...
  function openSandboxPopover() {
    ...
    // closed as before.
    if (!sandboxPopover.hidden && sandboxPopover.dataset.view === "picker") { closePopovers(); return; }
    closePopovers();
    sandboxPopover.classList.remove("sandbox-rules-popover");
    sandboxPopover.dataset.view = "picker";
    sandboxPopover.innerHTML = "";
    const cur = state.sandboxCurrent || "off";
    ...
    for (const option of options) {
      ...
      const scope = sandboxScopeFor(id, requestedScope);
      const scopeMeta = sandboxScopeMeta(scope);
    ...
    switch (msg.type) {
      ...
      case "sandboxState":
        ...
        state.sandboxProfiles = Array.isArray(msg.profiles)
          ...
          : [];
        state.sandboxRules = msg.rules && typeof msg.rules === "object" ? msg.rules : null;
  ...
  if (sandboxBtn) {
    ...
    sandboxBtn.addEventListener("mouseenter", () => {
      sandboxRulesButtonHovered = true;
      openSandboxRulesPopover();
    });
    sandboxBtn.addEventListener("mouseleave", () => {
      sandboxRulesButtonHovered = false;
      scheduleSandboxRulesPopoverClose();
    });
    sandboxBtn.addEventListener("focus", () => {
      openSandboxRulesPopover();
    });
  ...
  if (sandboxPopover) {
    sandboxPopover.addEventListener("click", (e) => e.stopPropagation());
    sandboxPopover.addEventListener("mouseenter", () => {
      sandboxRulesPopoverHovered = true;
      clearSandboxRulesHideTimer();
    });
    sandboxPopover.addEventListener("mouseleave", () => {
      sandboxRulesPopoverHovered = false;
      scheduleSandboxRulesPopoverClose();
    });
  }
