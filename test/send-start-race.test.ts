...
function makeSidebar(cwd: string): any {
  ...
  // globalState: the sandbox profile for the next spawn is resolved during
  // startSession and reads it, so a context without one crashes the start.
  const contextMemento: Record<string, unknown> = {};
  sidebar.context = {
    globalStorageUri: { fsPath: cwd },
    subscriptions: [],
    globalState: {
      get: (key: string, fallback?: unknown) =>
        Object.prototype.hasOwnProperty.call(contextMemento, key) ? contextMemento[key] : fallback,
      update: async (key: string, value: unknown) => { contextMemento[key] = value; },
    },
  };
...
describe("send vs concurrent startSession", () => {
  // GROK_HOME: sandbox profile selection falls back to $GROK_HOME/config.toml,
  // so a developer whose real config sets `[sandbox] profile` would have these
  // starts try to launch an actual Seatbelt broker and resolve undefined.
  let priorGrokHome: string | undefined;
  beforeEach(() => {
    priorGrokHome = process.env.GROK_HOME;
    process.env.GROK_HOME = mkdtempSync(path.join(tmpdir(), "grok-send-start-"));
  });
  afterEach(() => {
